...
describe('EnlargedSpider (RESULT-06)', () => {
  ...
  it('renders spider-chart when open=true', () => {
    ...
    // Dialog content is rendered via Radix portal to document.body, not the container.
  ...
  it('does not render spider-chart when open=false', () => {
    ...
    // Dialog closed — portal content is unmounted from document.body
    expect(document.querySelector('[data-testid="enlarged-spider"]')).toBeNull()
  })
  ...
  it('renders the same datasets as the small Spider (legend entry per dataset)', () => {
    ...
    // Spider legend has one entry per dataset (queried via document for Radix portal)
    const legend = document.querySelector('[data-testid="spider-legend"]')
